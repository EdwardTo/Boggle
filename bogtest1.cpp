...
int main (int argc, char* argv[]) {
  ...
  //TernaryTrie *p1 = new TernaryTrie();
  //string wordB("bree");
 // p1->add(wordB);
  set<string> lex1;
  lex1.insert("call");
  ...
  if(locations.size() != 3 || locations[0] != 0 || locations[1] != 4 || locations[2] != 1 ){// || locations[2] != 1 || locations[3] != 2 || locations[4] != 3) {
    for(int i = 0; i < locations.size(); i++)
    std::cerr << "Size: " << locations.size() << " location: " << locations[i] << std::endl;
    std::cerr << "Location: " << locations[1] << std::endl;
  ...
  if(words.size() != 5){// || words.count(wordA) != 1) {
    std::cerr << "Apparent problem with getAllValidWords #2." << std::endl;
    std::cerr << "size: " << words.size() << std::endl;
    return -1;
  }
    int i=0;

    for(it = words.begin(); it!=words.end(); ++it){
      std::cerr << "words[" << i << "]: " << *it << std::endl;
      i++;
    }

    string rowQ[] = {"A"};
    string *diceArray2[] = {rowQ};
    p->setBoard(1, 1, diceArray2);
    vector<int> empty;
    string testString = "A";
    locations.clear();
    locations.push_back(0);
    vector<int> blah;
    if (p->isOnBoard(testString) != locations)
    {
        cout << "ERROR: \"A\" was not in the right place on the board in the 1x1 test " << endl;
        blah = p->isOnBoard(testString);
        cout << "positions: " << locations[0] << "positions size: " << locations.size() << endl;
        cout << "position: " << blah[0] << "Blah size: " << blah.size() << endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        cout << "ERROR: \"A\" was not in the right place on the board in the 1x1 test " << endl;
        blah = player->isOnBoard(testString);
        cout << "positions: " << positions[0] << "positions size: " << positions.size() << endl;
        cout << "position: " << blah[0] << "Blah size: " << blah.size() << endl;
        failed = true;
    }
